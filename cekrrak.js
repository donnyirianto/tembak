...
const cekToko = async () => {
  try {    
    ...
    if(rpreal>0 and rprenc>0 and ftrf_true>0 and fvalid_true>0,'Sudah Realisasi Sudah Transfer',
    if(rpreal>0 and rprenc>0 and ftrf_true=0 and fvalid_true>0,'Sudah Realisasi Belum Transfer',
        if(rpreal=0 and rprenc>0 and ftrf_true=0 and fvalid_true>0,'Sudah Validasi Belum Realisasi',
            if(rpreal=0 and rprenc>0 and ftrf_true=0 and fvalid_true=0,'Sudah Input Belum Validasi',
                if(rprenc=0 or rprenc is null,'Belum Input RRAK','Blank'
                )
            )
        )
    )    
