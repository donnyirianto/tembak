    ...
    try{  
      ...
      fileReport.once('open', async function(fd) { 
        ...
        results.forEach( async (r) => { 
          ...
          const queryTembak = `SELECT  '${r.kdcab}' as kdcab, '${r.kdtk}' as kdtk, '${r.nama}' as nama_toko,
          (SELECT FILTER FROM VIR_BACAPROD WHERE JENIS = 'ONLINETBM') AS TBM, 
          (SELECT GROUP_CONCAT(JENIS) AS JENIS FROM const where rkey='PCO') AS PCO,
          (select nilai from program_setting where jenis  = 'INTERVALGETTASK') AS INTERVALGETTASK;`;
          await Models.vquery(r.ip1, "UPDATE CONST SET JENIS = 'Y' WHERE RKEY='PCO'")
